...
export class HomeComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    ngAfterViewInit() {
        if (this.mapPrepared) {
            this.addPolygon();
            this.restapi.getHomePagePOIs().subscribe(s => {
                this.markers = s;
    ...
    callPhoneNumber(){
        TNSPhone.requestCallPermission('Θα πρέπει να αποδεχτείτε την άδεια για να μπορείτε να πραγματοποιήσετε απευθείας τηλεφωνική κλήση.')
            .then(() => TNSPhone.dial('2104990500', false))
            .catch(() => TNSPhone.dial('2104990500', true));
    ...
    static getMarkers(): MapboxMarker[] {
        ...
        this.data.forEach(f => {
            const mark = {
                ...
                title: 'Απινιδωτής',
                subtitle: f.title,
                icon: 'https://cp.leadersoft.gr/assets/images/hospital_128.png',
            };
            markers.push(mark);
            idx++;
        });
        const wifimark = {
            ...
            icon: 'https://cp.leadersoft.gr/assets/images/pin_wifi.png',
        };
